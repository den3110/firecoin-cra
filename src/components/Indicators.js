...
const OsGauge = React.lazy(() => import("@/components/gauges/OsGauge"));
const SuGauge = React.lazy(() => import("@/components/gauges/SuGauge"));
const MaGauge = React.lazy(() => import("@/components/gauges/MaGauge"));
...
const Indicators = () => {
  ...
  useEffect(() => {
    ...
    SocketClient.getInstance()
      .socket()
      .on("BO_CHART_INDICATORS", handleIndicators);

    return () => {
      SocketClient.getInstance()
        .socket()
        .emit("BO_CHART_INDICATORS_UNSUBSCRIBE");
      SocketClient.getInstance()
        .socket()
        .off("BO_CHART_INDICATORS", handleIndicators);
    };
  ...
  const result = useMemo(() => {
    ...
    oscillators.items.relativeStrengthIndex = convertTextState(
      indicators.rsi[1]
    );
    oscillators.items.stochasticOscillator = convertTextState(indicators.so[1]);
    oscillators.items.commodityChannelIndex = convertTextState(
      indicators.cci[1]
    );
    ...
    oscillators.items.williamsPercentRange = convertTextState(
      indicators.wpr[1]
    );
    oscillators.items.ultimateOscillator = convertTextState(indicators.uo[1]);

    movingAverages.items.exponentialMovingAverage5 = convertTextState(
      indicators.ema_5[1]
    );
    movingAverages.items.exponentialMovingAverage10 = convertTextState(
      indicators.ema_10[1]
    );
    movingAverages.items.exponentialMovingAverage20 = convertTextState(
      indicators.ema_20[1]
    );
    movingAverages.items.exponentialMovingAverage30 = convertTextState(
      indicators.ema_30[1]
    );
    movingAverages.items.exponentialMovingAverage50 = convertTextState(
      indicators.ema_50[1]
    );
    movingAverages.items.exponentialMovingAverage100 = convertTextState(
      indicators.ema_100[1]
    );
    movingAverages.items.simpleMovingAverage5 = convertTextState(
      indicators.sma_5[1]
    );
    movingAverages.items.simpleMovingAverage10 = convertTextState(
      indicators.sma_10[1]
    );
    movingAverages.items.simpleMovingAverage20 = convertTextState(
      indicators.sma_20[1]
    );
    movingAverages.items.simpleMovingAverage30 = convertTextState(
      indicators.sma_30[1]
    );
    movingAverages.items.simpleMovingAverage50 = convertTextState(
      indicators.sma_50[1]
    );
    movingAverages.items.simpleMovingAverage100 = convertTextState(
      indicators.sma_100[1]
    );
